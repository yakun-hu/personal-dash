...
<head><title>Backlog-manager:</title><link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Merriweather">  <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Comfortaa">  <link rel="stylesheet" href="../../php_local_libs/CSS-external.php">
<style>.l1 { grid-area: simul-top; background-color: #FFFFFF; border-style: solid; border-width: 3px; border-color: #1F1F1F;}
...
.grid-container { display: grid; grid: 'simul-top nadel-top'; grid-gap: 1px; padding: 10px; height: 400px; width: 1000px; grid-template-rows: 400px; grid-template-columns: 1fr 1fr; text-align: left;}
.grid-container > div { padding-top: 10px; padding-left: 10px; margin-right: 10px; } 
table {border-collapse: collapse; width: 100%;}
td { border: 1px solid #dddddd; text-align: left; padding: 8px; font-family: Arial; font-size: 10.666px; font-style: normal; font-variant: normal; font-weight: normal; line-height: 18.4px; margin-top: 0px; margin-bottom: 0px;}
th { border: 1px solid #dddddd; text-align: left; padding: 8px;}</style></head>
...
<form method="post"><p>Task-title: <input required type="text" name="task_title"></input><span style='color:red'>* </span>Due: <input required type="date" name="due_date_f" value="<?php echo date('Y-m-').date('d')+3;?>"><br>
Task-type: <input style="vertical-align:sub;" type="checkbox" name="prom">Promise<input type="checkbox" style="vertical-align:sub" name="phi">Philo<input type="checkbox" name="m_kats" style="vertical-align:sub">＜-Micro.Kats<input type="checkbox" name="big" style="vertical-align:sub"><a href="https://www.upwork.com/freelancers/~0165d626c768f8f3fe/" target="_blank">大</a><input type="checkbox" name="rel" style="vertical-align:sub">Rel<br>
...
<div style="margin-top:2px"><?php /*span-8*/require'../../php_local_libs/edit.panel-inc.php';if(isset($_POST["task_title"])){ins_assembl_sql();}?><input style="display:inline;background-color:944761" type="submit" value="Save"></div></form></div>
...
<h4>Project-summary:</h4><p>Calculations: preview if a synchro/migr task will have enough free space on the receiving side; calculate the "synchro-margin" and run an if-comp/a/r w/ that free-space # |
 WordPress[<a href="https://automattic.com/about/" target="_blank">Automattic</a>], Gutenberg, and Woo-Commerce</div></div>
<h2>Back-log:</h2>
<!--h4--><h4><div id="布告" style="display:flex;height:30px;line-height:30px;vertical-align:middle;text-align:center"><div style="flex-grow:1;background-color:2F3A84;align-self:center"><a href="后卫.php?板=prom&pg_数=1#布告" style="color:B13B3A">Promise</a></div><div style="flex-grow:1;background-color:F0B5B5"><a href="后卫.php?板=phi&pg_数=1#布告" style="color:FFF0CB">Philo</a></div><div style="flex-grow:1;background-color:38761D"><a href="后卫.php?板=sort&pg_数=1#布告" style="color:F1C232">Sort</a></div><div style="flex-grow:1;background-color:3C5977"><a href="后卫.php?板=rel&pg_数=1#布告" style="color:F2F0D3">Rel</a></div><div style="flex-grow:1;background-color:125274"><a href="后卫.php?板=m_kats&pg_数=1#布告" style="color:C79E5C">Micro-Kats</a></div><div style="flex-grow:1;background-color:E80508"><a href="后卫.php?板=Big&pg_数=1#布告" style="color:F1B70F">Big</a></div></div></h4>
<!--tbleau-set--><table><tr><th>task_title</th><th>tech_notes</th><th style="width:70%">description:</th><th>status</th><th>due⏱️</th></tr>
<!--concise-loader➡️--><?php $db_obj=select_assembl_后卫(); ?><!--loop-meta--><?php $计数=1;$计结='假';$介于_首位=intval($_GET['pg_数'].'1')-10;/*<setters-loop->*/while($这排=$db_obj->fetch_assoc()){$计数=$计数+1;if(0==0){?>
...
</select><span id="save_dr<?php echo$这排['post_ID'];?>"></span><button id="conf<?php echo$这排['post_ID'];?>" hidden onclick="status_exec()">Conf-del</button>

</td><td><input type="date" value="<?php echo$这排["deadline_date"];?>" onchange="status_exec()"></td>
...
<!--loop-clos--><?php }if($计数==intval($介于_首位+9)){$计结='true';}}?></table><!--paginators--><p><?php if(intval($_GET['pg_数'])!==1) {?><a href="<?php echo '后卫.php?板='.$_GET["板"].'&pg_数='.intval($_GET['pg_数'])-1 .'#布告';?>">Prev-page | </a><?php }?> <?php if($计结 !== '假'){?><a href="<?php echo '后卫.php?板='.$_GET["板"].'&pg_数='.intval($_GET['pg_数'])+1 .'#布告';?>">Next-page</a><?php }?></p>
<!--ref-URL: http://personal-dash/testing-progress/publications/后卫.php?板=Promise&pg_数=1#布告 -->
...
function/*analy-2*/select_assembl_后卫(){$行_列=array("*");$指标行列=array("attributes");$指标经营列=array("LIKE");$指标val列=array("'%{$_GET["板"]}=true%'");$order=array("post_ID","DESC");
/*call-SQL,inc*/ return 选出_rows_any("to_do_blind",$行_列,$指标行列,$指标经营列,$指标val列,$order);}?>
<script>function status_exec(){if(event.target.type=="date"){var 执行载荷=event.target.value;console.log(执行载荷);}
else {let 地位/*status-obj*/=event.target;var post_ID=地位.id.slice(11);console.log(post_ID);
if(地位.value=="in_prog"||地位.value=="mark_compl"){var 执行载荷/*message.to-PHP*/=document.getElementById("attr"+post_ID).innerHTML+"|status="+地位.value+";|"+post_ID;console.log(执行载荷);}
else if(地位.value=="del"){地位.hidden=true;document.getElementById("conf"+post_ID).hidden=false;return;/*terms-avoi,AJAX*/}}
/* Ajax: 准并-开火 */let 包裹=new XMLHttpRequest();包裹.open("POST","../../php_local_libs/mysql.query-inc.php",true);包裹.setRequestHeader("Content-type", "application/x-www-form-urlencoded");包裹.send("payload="+执行载荷); 
/*resp-displ*/包裹.onload=function(){console.log("hey");document.getElementById("save_dr"+post_ID).innerHTML+=this.responseText;}}</script>
<!-- testing: [Late.6th-22]
...
<!-- notes: 
[Late-6th,21]ordr*the tbl.r-1 is within the while-l, so when # there is 0 loops, there is no echo, when-in
===
...
5) Input form from dash-edit: expandable pop-up, in-line, onfocus, test # 
	...
	Use cases:
		...
		Build in lazy losing and caching concepts to backlog-man working.MVP-2
...
<script>document.getElementById("bold").style = "position:absolute;top:348px;left:172px;background-color:black;height:30px;color:#FFFFFF";
document.getElementById("italic").style = "position:absolute;top:348px;left:194px;background-color:blue;height:30px;color:white";
document.getElementById("underline").style = "position:absolute;top:348px;left:212px;background-color:00FF00;height:30px;color:white"; console.log("test"); // console.log is echoing to console #<ya-welcom></script>
